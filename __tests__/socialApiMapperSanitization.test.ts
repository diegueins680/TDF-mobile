...
describe('Social API mapper sanitization', () => {
  ...
  it('Events.reactToMoment sends an idempotent desired reaction state', async () => {
    ...
    post.mockResolvedValueOnce({
      ...
      emReactions: [{ emrReactionTypeId: reactionTypeId, emrPartyId: '7' }],
      emComments: [],
    });
    ...
    expect(post).toHaveBeenCalledWith(
      '/social-events/events/42/moments/77/reactions',
      { emrrReactionTypeId: reactionTypeId, emrrActive: true },
    );
