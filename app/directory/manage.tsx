...
function ProfileForm({ taxonomies, onCreated }: { taxonomies: Taxonomies; onCreated: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: () => {
      ...
      return Directory.createProfile({
        ...
        cityId, onsite: true, remote, availableToTravel: false,
      });
    },
    onSuccess: onCreated,
  ...
  return (
    <View style={[styles.form, { backgroundColor: colors.surfaceRaised, borderColor: colors.borderSubtle }]}>
      ...
      <View style={styles.row}><Text style={{ color: colors.textPrimary }}>También trabajo remoto</Text><Switch value={remote} onValueChange={setRemote} /></View>
      <Pressable accessibilityRole="button" disabled={!name.trim() || mutation.isPending} style={[styles.primaryButton, { backgroundColor: colors.actionPrimary, opacity: !name.trim() || mutation.isPending ? 0.6 : 1 }]} onPress={() => mutation.mutate()}><Text style={{ color: colors.actionPrimaryContrast, fontWeight: '800' }}>Guardar borrador</Text></Pressable>
    </View>
  );
}
...
function ClassifiedForm({ taxonomies, profiles, onCreated }: { taxonomies: Taxonomies; profiles: Awaited<ReturnType<typeof Directory.managedProfiles>>; onCreated: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: () => Directory.createClassified({
      ...
      metropolitanAreaIds: [], onsite: cityIds.length > 0, remote, availableToTravel: false, budgetNegotiable: false,
    }),
    onSuccess: onCreated,
  ...
  return (
    <View style={[styles.form, { backgroundColor: colors.surfaceRaised, borderColor: colors.borderSubtle }]}>
      ...
      <View style={styles.row}><Text style={{ color: colors.textPrimary }}>Acepta remoto</Text><Switch value={remote} onValueChange={setRemote} /></View>
      ...
      <Pressable accessibilityRole="button" disabled={!profileId || !categoryId || title.trim().length < 5 || description.trim().length < 20 || (!remote && !cityIds.length) || mutation.isPending} style={[styles.primaryButton, { backgroundColor: colors.actionPrimary }]} onPress={() => mutation.mutate()}><Text style={{ color: colors.actionPrimaryContrast, fontWeight: '800' }}>Guardar borrador</Text></Pressable>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  fieldLabel: { fontWeight: '800' }, chips: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 }, segment: { minHeight: 42, borderWidth: 1, borderRadius: 22, paddingHorizontal: 13, alignItems: 'center', justifyContent: 'center' },
});
